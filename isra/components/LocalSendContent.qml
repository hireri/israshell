...
Item {
    ...
    Item {
        ...
        x: {
            if (!root.controller)
                return 0;
            const widgetItem = root.controller.widgetItem;
            const mapped = widgetItem.mapToItem(root, widgetItem.width / 2, 0);
            const idealX = mapped.x - (root.cardW / 2);
        ...
        ClippingRectangle {
            ...
            y: {
                ...
                const edgeMargin = root.controller?.panelWindow.barHeight ?? 0;
